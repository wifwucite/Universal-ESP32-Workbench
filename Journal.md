...
## 26-04-13 (Session 1 - ESP32-H2 chip revision fix and board pin mapping)

### Summary

Fixed ESP32-H2 bootloader incompatibility with rev v1.2+ silicon by enabling the H2 v1.x preview flag in persistent sdkconfig defaults. Identified correct RGB LED GPIO pin (8) for Waveshare ESP32-H2-Zero board.

### What we did

- **Chip revision issue**: Discovered that pre-built H2 binaries were capped to chip rev v0.99 due to:
  - `sdkconfig` encoding `CONFIG_ESP32H2_REV_MAX_FULL=99` (v0.99).
  - ESP-IDF v5.4 enforces a hard limit of 99 unless `CONFIG_ESP32H2_REV100_DEVELOPMENT=y` is set.
  - Solution: Created persistent defaults files to lock in the v1.x preview flag:
    - `sdkconfig.defaults`: Cross-target debug settings (debug optimization).
    - `sdkconfig.defaults.esp32h2`: H2-specific settings with `CONFIG_ESP32H2_REV100_DEVELOPMENT=y`.
  - After rebuilding, `CONFIG_ESP32H2_REV_MAX_FULL` now resolves to 199 (v1.99), allowing v1.2 silicon.

- **Board pin mapping**: Researched Waveshare ESP32-H2-Zero onboard LED:
  - Found WS2812 RGB LED connected to **GPIO8**.

- **sdkconfig behavior**: Clarified that:
  - `sdkconfig` is auto-generated by Kconfig and should not be manually edited.
  - Persistent config must go in `sdkconfig.defaults` (and target-specifics like `sdkconfig.defaults.esp32h2`).
  - Both are version-controlled; build scripts rely on them.

### Learnings

- ESP-IDF often gates newer chip silicon revisions behind preview/development flags, especially for recently released variants like H2.
- For H2, the encoding formula is: `REV_FULL = major*100 + minor` (e.g., 102 = v1.2, 199 = v1.99).
- Onboard LED pins vary by board reference design (Espressif vs. Waveshare), so always verify the vendor datasheet/wiki rather than assuming consistency across variants.
- Using persistent `sdkconfig.defaults*` files is the only reliable way to preserve config across `idf.py fullclean` and multi-target builds.
